...
exports.WAConnection = (_WAConnection) => {
  class WAConnection extends _WAConnection {
    constructor(...args) {
      super(...args);
      if (!Array.isArray(this._events["CB:action,add:relay,message"]))
        this._events["CB:action,add:relay,message"] = [
          this._events["CB:action,add:relay,message"]
        ];
      else
        this._events["CB:action,add:relay,message"] = [
          this._events["CB:action,add:relay,message"].pop()
        ];
      this._events["CB:action,add:relay,message"].unshift(async function (
        json
      ) {
        try {
          let m = json[2][0][2];
      ...
      this.on(`CB:action,,battery`, (json) => {
        this.battery = Object.fromEntries(
          Object.entries(json[2][0][1]).map((v) => [v[0], eval(v[1])])
        );
      });
    ...
    async copyNForward(jid, message, forceForward = false, options = {}) {
      ...
      let content = await this.generateForwardMessageContent(
        message,
        forceForward
      );
      ...
      const waMessage = await this.prepareMessageFromContent(
        jid,
        content,
        options
          ? {
    ...
    async genOrderMessage(message, options) {
      ...
      switch (type) {
        ...
        case MessageType.extendedText:
          ...
          m.extendedTextMessage = WAMessageProto.ExtendedTextMessage.fromObject(
            message
          );
          break;
        case MessageType.location:
        case MessageType.liveLocation:
          m.locationMessage = WAMessageProto.LocationMessage.fromObject(
            message
          );
          break;
        ...
        case MessageType.contactsArray:
          m.contactsArrayMessage = WAMessageProto.ContactsArrayMessage.fromObject(
            message
          );
          break;
        case MessageType.groupInviteMessage:
          m.groupInviteMessage = WAMessageProto.GroupInviteMessage.fromObject(
            message
          );
          break;
    ...
    async sendContact(jid, number, name, quoted, options) {
      ...
      let { isBusiness } = (await this.isOnWhatsApp(njid)) || {
        isBusiness: false
      };
      let vcard = `
BEGIN:VCARD
...
X-WA-BIZ-DESCRIPTION:${(
              (await this.getBusinessProfile(njid)).description || ""
            ).replace(/\n/g, "\\n")}
`
          : ""
      }
...
`.trim();
    ...
    async prepareMessageMedia(buffer, mediaType, options = {}) {
      ...
      if (
        !(mediaType === MessageType.image || mediaType === MessageType.video) &&
        options.viewOnce
      ) {
      ...
      const requiresThumbnailComputation =
        (mediaType === MessageType.image || mediaType === MessageType.video) &&
        !("thumbnail" in options);
      const requiresDurationComputation =
        mediaType === MessageType.audio && !options.duration;
      ...
      const mac = hmacSign(
        Buffer.concat([mediaKeys.iv, enc]),
        mediaKeys.macKey
      ).slice(0, 10);
      ...
      for (let host of json.hosts) {
        ...
        try {
          ...
          const isLast =
            host.hostname === json.hosts[json.hosts.length - 1].hostname;
          this.logger.error(
            `Error in uploading to ${host.hostname}${
              isLast ? "" : ", retrying..."
            }`
          );
        }
    ...
    async sendButton(
      ...
      return await this.sendMessage(
        ...
        {
          contextInfo: { mentionedJid: this.parseMention(content + footer) },
          quoted,
          ...options
        }
      );
    ...
    async sendButtonImg(
      ...
      return await this.sendMessage(
        jid,
        {
          ...
          headerType: 4,
          imageMessage: (
            await this.prepareMessageMedia(file, MessageType.image, {})
          ).imageMessage
        },
        MessageType.buttonsMessage,
        {
    ...
    async sendButtonVid(
      ...
      return await this.sendMessage(
        jid,
        {
          ...
          headerType: 5,
          videoMessage: (
            await this.prepareMessageMedia(file, MessageType.video, {})
          ).videoMessage
        },
        MessageType.buttonsMessage,
        {
    ...
    async sendListM(jid, button, rows, quoted, options) {
      ...
      let waMessageList = await this.prepareMessageFromContent(
        jid,
        messageList,
        {
          quoted,
          contextInfo: {
            mentionedJid: this.parseMention(button.description),
            ...options
          }
        }
      );
    ...
    async sendFile(
      ...
      if (options.asSticker) mtype = MessageType.sticker;
        ...
        if (/webp/.test(type.mime) && file.length <= 1 << 20)
          ...
          try {
            ...
            console.error(
              "Error send video using sendVideo, retrying using sendMessage... ",
              e
            );
        ...
        if (options.asGIF && mtype === MessageType.video)
    ...
    reply(jid, text, quoted, options) {
      return Buffer.isBuffer(text)
        ...
        : this.sendMessage(jid, text, MessageType.extendedText, {
            contextInfo: { mentionedJid: this.parseMention(text) },
            quoted,
            ...options
          });
    }
    ...
    async downloadM(m) {
      ...
      if (!m.message[Object.keys(m.message)[0]].url)
        await this.updateMediaMessage(m);
...
exports.smsg = (conn, m, hasParent) => {
  ...
  if (m.message) {
    ...
    if (m.quoted) {
      ...
      if (m.quoted.url)
        m.quoted.download = (type = "buffer") => conn.downloadM(vM, type);
      /**
    ...
    m.forward = (jid = m.chat, forceForward = false) =>
      conn.forwardMessage(jid, m, forceForward);
    /**
...
async function download(media, mime, callback) {
  if (Buffer.isBuffer(media)) {
    if (typeof callback == "function")
      await callback({ buffer: media, filename: "" });
    return media;
  }
    ...
    try {
      ...
      await res.body
        ...
        .on("finish", async () => {
          buffer = await fs.readFileSync(filename);
          if (typeof callback == "function")
            await callback({ buffer, filename });
        });
      if (fs.existsSync(filename)) await fs.unlinkSync(filename);
      return filename;
    } catch (e) {
